<div x-data="
    ...
    <template x-for="(star, index) in ratings" :key="index">
        <button @click="window.livewire.emit('rate',{ amount : star.amount });" @mouseover="hoverRating = star.amount" @mouseleave="hoverRating = rating"
                aria-hidden="true"
                :title="star.label"
                class="rounded-sm text-slate-300 fill-current focus:outline-none focus:shadow-outline p-1 w-8 m-0 cursor-pointer"
                :class="{'text-yellow-300': hoverRating >= star.amount || star.amount <= rating, 'text-yellow-400': rating >= star.amount && hoverRating >= star.amount}">
